...
const experiences = [
  {
    ...
    details: [
      ...
      {
        text: "나노종합기술원, 한국광기술원, KIST(한국과학기술연구원) 통합관리시스템(MIS)와 연계한 신규 MRO 쇼핑몰 오픈 및 유지보수",
        subDetails: ["신규 오픈하는 고객사를 전담하여 온라인 쇼핑몰 3회 오픈 ( Spring 전자정부프레임워크 + javascript )", 
          "보안 상 기관 데이터베이스에 직접 접근할 수 없어 중계서버를 이용해 데이터를 교환하는 방식을 사용(MIS <- 중계서버 -> 쇼핑몰)",
          "쇼핑몰 ) 오픈 전 고객사 요구사항 개발, 서버 WAS 설치 및 소스 배포 ",
          "중계서버 ) 데이터 송수신부를 개발, 서버 WAS 설치 및 소스 배포 ",
          " MIS와 쇼핑몰 간 SSO 기능을 추가하며, SSO 업체와 협업하여 SSO 라이브러리 서버 설치, 가이드 참고하여 로그인 수신부 개발",
          " MIS 개발팀(타사)과 협업하여 오픈전 단위별, 어플리케이션 간 통합 테스트 진행"
